...
def extract_fault_name(name: str) -> str:
    ...
    for kw in ["第一類活動斷層地質敏感區", "第二類活動斷層地質敏感區",
               "第一類活動斷層", "第二類活動斷層", "活動斷層地質敏感區"]:
        name = name.replace(kw, "")
    return name.strip()


def geom_to_wgs84_bbox_center(geom):
    """將 TWD97 geometry 轉換，回傳 (bbox, center_lng, center_lat, geojson_str)"""
    # 重投影到 WGS84
    geom_wgs84 = gpd.GeoSeries([geom], crs="EPSG:3826").to_crs("EPSG:4326").iloc[0]
...
def process_shp(path: Path, out_f: io.TextIOWrapper, stats: dict) -> None:
    try:
    ...
    for _, row in gdf.iterrows():
        ...
        fault_class = infer_fault_class(raw_name)
        fault_name = normalize_tw(extract_fault_name(raw_name))
        if not fault_name:
            fault_name = normalize_tw(raw_name)

        try:
            bbox, center_lat, center_lng, geojson_str = geom_to_wgs84_bbox_center(row.geometry)
        except Exception as e:
        ...
        stmt = (
            ...
            f"{center_lat},{center_lng},'{esc(geojson_str)}',"
